...
def compress(s):
    ...
    u={symbol:i for i,symbol in enumerate(symbols)}
    rev_symbols=list(reversed(symbols[:len(most_common_words)]))
    ...
    for word in new_words:
        if word.count("\n") == 1:
            ...
            nn.append(word)
            i += 1
    d={value:i for i,value in enumerate(rev_symbols)}
    y=[]
    nnn=[]
    for _nn in nn:
        if _nn in d:
            if d[_nn]>5000:
                nnn.append(symbols[d[_nn]-5000])
                y.append("0")
            else:
                y.append("1")
    v = C1.join([
    ...
    return zstd.compress(v.encode("utf-8", "replace"),level=22)
